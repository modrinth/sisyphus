...
async fn count_download(req: &Request, ctx: &RouteContext<()>) -> Result<()> {
	...
	let downloader_downloads = downloaders.get(&ip).bytes().await?
	    .map(|it| u32::from_le_bytes(it[0..4].try_into().unwrap()))
	    .unwrap_or(0);

	let downloader_dl_increment_future = downloaders.put_bytes(&ip, &u32::to_le_bytes(downloader_downloads + 1))?
...
const URL_PARAM_ERROR: &str = "Tried to get nonexistent file, the router should not have matched this route!";
...
fn get_file(ctx: &RouteContext<()>) -> Result<Response> {
    let (hash, version, file) = (get_param(ctx, "hash"), get_param(ctx, "version"), get_param(ctx, "file"));
